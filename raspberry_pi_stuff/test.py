...
def find_placer(txt):
    ...
    if place != 0:
        ...
        raise ValueError('Your value {} is not in the classification scheme'.format(txt))
...
def get_frame(this_frame, this_model, placer):
...
def run(text):
    ...
    time.sleep(2.0)
    #lets load the placer of the text
    placer = find_placer(text)
    # loop over the frames from the video stream
    while True:
        # grab the frame from the threaded video stream and resize it
        # to have a maximum width of 400 pixels
        frame_in = vs.read()
        frame = get_frame(frame_in, model, placer)
        frame2 = get_frame(frame_in, original_model, placer)
...
if __name__ == '__main__':
    ...
    ap.add_argument("-t", "--text", type=str, required=True, help="image to classify on")
